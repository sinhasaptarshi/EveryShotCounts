...
def save_exemplar(dataloaders, model, args):
    ...
    target_dir = f'exemplar_{args.model}tokens_{args.dataset}_ego4d_pretrained'
    for split in splits:
...
def save_tokens(dataloaders, model, args):
    ...
    target_dir = f'saved_{args.model}tokens_{args.dataset}_ego4d_pretrained'
...
def main():
    ...
    if args.model == 'VideoMAE':
        ...
        model = model.cuda()
        model.load_state_dict(torch.load('resnext101.pth')['state_dict'], strict=False)
        model = nn.DataParallel(model, device_ids=[i for i in range(args.num_gpus)])
    ...
    if args.dataset == 'RepCount':
        ...
        dataset_train = EgoLoops(cfg=cfg,split="train",sampling_interval=1,encode_only=True)
        dataset_val = EgoLoops(cfg=cfg,split="val",sampling_interval=1,encode_only=True)
        dataset_test = EgoLoops(cfg=cfg,split="val",sampling_interval=1,encode_only=True)
