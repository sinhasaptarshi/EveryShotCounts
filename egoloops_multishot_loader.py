...
class EgoLoops(torch.utils.data.Dataset):
    def __init__(self,
                 split="val",
                 add_noise= False,
                 num_frames=512,
                 tokens_dir = "saved_VideoMAEtokens_EgoLoops_ego4d_pretrained",
                 exemplar_dir = "exemplar_VideoMAEtokens_EgoLoops_ego4d_pretrained",
        ...
        if self.split == 'train':
            ...
            csv_path = f"datasets/ucf-rep/ucf_aug_train.csv"
            # self.train_data = pkl.load(open('../train_ucf_aug.pkl', 'rb'))
        else:
            csv_path = f"annotated_egoloops.csv"
...
# integral f(x)
    ...
    def __getitem__(self, index):
        ...
        video_name = self.df.iloc[index]['name'].split('/')[-1].replace('.mp4', '.npz')
        ...
        if self.split == 'train':
            # starts = cycle[0::2]
            # ends = cycle[1::2]
            # st = np.random.randint(0, len(starts))
            # end = np.random.randint(st, len(ends))
            # segment_start = int(starts[st])
            ...
            segment_start = 0
            segment_end = min(row['total_duration'],900)
        num_frames = math.ceil(min(row['total_duration'], 900)/self.temporal_downsample)* self.temporal_downsample  
...
if __name__=='__main__':
    from tqdm import tqdm
    dat = EgoLoops(select_rand_segment=False, compact=False, pool_tokens_factor=0.5, get_overlapping_segments=False)
