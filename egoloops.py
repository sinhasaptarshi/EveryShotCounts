...
class EgoLoops(torch.utils.data.Dataset):
    def __init__(self,
                 ...
                 encode_only=False,
                 sampling_interval=4,
                 data_dir = "/jmain02/home/J2AD001/wwp01/sxs63-wwp01/repetition_counting/CounTR/selected_videos/"):
        ...
        self.videos = os.listdir(self.data_dir)
    ...
    def __getitem__(self, index):
        ...
        video_name = f"{self.data_dir}/{self.df.iloc[index]['name']}"
        #print(os.path.isfile(video_name))
        cap = cv2.VideoCapture(video_name)
        fps = cap.get(cv2.CAP_PROP_FPS)
        
        row = self.df.iloc[index]
        duration = min(int(cap.get(cv2.CAP_PROP_FRAME_COUNT)), 900)
        ...
        frame_idx = self.get_vid_clips(min(duration-1, 900))
        ...
        vdur = (frame_idx[-1] - frame_idx[0]) / fps
