...
class Rep_count(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        mean_duration = 512
        # video_path = f"{self.data_dir}/{self.split}/{self.df.iloc[index]['name']}"
        examplar_path = f"examplar_tokens/{video_name}"
        examplar = np.load(examplar_path)['arr_0']
        N = examplar.shape[0]
        if self.split == 'train':
            idx = np.random.randint(N)
        else:
            idx = 0
        example_rep = torch.from_numpy(examplar[idx:idx+1])  ### return the encoding for a selected example per video instance
        example_rep = example_rep.reshape(1, 768, -1).permute(0,2,1)
        del examplar

        #print(os.path.isfile(video_name))

        ## loading density maps
        gt_density_map = np.load(f"gt_density_maps/{video_name}")['arr_0']
        gt_density = gt_density_map[0::4] ## downsample by 4
        gt_density = gt_density/gt_density.sum() * row['count']  ##scale by count to make the sum consistent
        # print(gt_density.sum())

        
        clc = np.array([int(float(row[key])) for key in row.keys() if 'L' in key and not np.isnan(row[key])])
        starts = clc[0::2]
        ends = clc[1::2]
        segment_mids = (starts + ends)//2
        
        count = row['count']
        video_features_path = f"saved_tokens/{video_name}"
        video_encodings = np.load(video_features_path)['arr_0']
        vid = video_encodings[0::4] ### for non overlapping segments
        del video_encodings
        ...
        if T <= 512:
            ...
            start = random.choice(np.arange(0, T-512, 64))
            end = start + 512  ## for taking 8 segments

        sampled_segments = torch.from_numpy(vid[(start//64) : (end//64)])
