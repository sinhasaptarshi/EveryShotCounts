...
if __name__ == '__main__':
    ...
    input = torch.rand(2, 3, 64, 224, 224)
    model = resnet101(num_classes=400, sample_size=224, sample_duration=64, last_fc=False)
    y = model(input)
    # print(y)
    model = torch.nn.DataParallel(model)
    model.load_state_dict(torch.load('../resnext-101-kinetics.pth', map_location='cpu')['state_dict'], strict=True)
    y1 = model(input)
    print(y1.shape)
    # print(model)