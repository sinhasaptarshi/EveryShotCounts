...
class SupervisedMAE(nn.Module):
    def __init__(self, cfg=None, img_size=384, patch_size=16, in_chans=3,
                 ...
                 use_precomputed=True, token_pool_ratio=1.0, iterative_shots=False, encodings='swin', finetune_encoder=False):
    ...
    def forward(self, vid, yi=None, thw=None, boxes=None, shot_num=1, start=0, end=None):
        # if boxes.nelement() > 0:
        ...
        if not self.use_precomputed and self.finetune_encoder:
            if self.encodings == 'mae':
                ...
                # print(latent.shape)
                if shot_num > 0:
                    exemplars = latent[-shot_num:]
                    # print(exemplars.shape)
                    exemplars = einops.rearrange(exemplars, 'b c t h w -> (b t) (h w) c')
                    exemplars = einops.rearrange(exemplars, 't s c -> (t s) c')
                    latent = latent[:-shot_num]
                    # print(exemplars.shape)
                latent = einops.rearrange(latent, 'b c t h w -> (b t) (h w) c')
                if end is not None:
                    low_bound = math.floor(start/16)
                    # up_bound = bounds[1]//8 
                    up_bound = min(math.ceil(end/16), 150)
                    # print(latent.shape)
                    latent = latent[low_bound:up_bound]
                latent = einops.rearrange(latent, 't s c -> (t s) c')
                del vid
        ...
        # print(thw[0])
        # print(latent.shape)
        x = self.decoder_embed(latent.unsqueeze(0))
        # print(x.shape)
        # x = latent
        # x = x + self.decoder_pos_embed
        ...
        # print(shot_num)
        if shot_num > 0:
            yi = self.decoder_embed(exemplars.unsqueeze(0))
            N,_,C = yi.shape
