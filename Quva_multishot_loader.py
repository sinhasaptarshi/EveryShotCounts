...
class Quva(torch.utils.data.Dataset):
    def __init__(self,
                 split="train",
                 add_noise= False,
                 num_frames=512,
                 tokens_dir = "saved_3D-ResNeXt101tokens_Quva_ucfpretrained",
                 exemplar_dir = "exemplar_3D-ResNeXt101tokens_Quva_ucfpretrained",
        ...
        if self.split == 'train':
            ...
            # csv_path = f"datasets/repcount/{self.split}_balanced_new.csv"
            csv_path = f"datasets/quva/{self.split}_with_fps.csv"
        else:
            csv_path = f"datasets/quva/new_test.csv"
...
# integral f(x)
    ...
    def load_tokens(self,path,is_exemplar,bounds=None, lim_constraint=np.inf, id=None, cycle_start_id=0, count=None, get_overlapping_segments=True, merge_early=False, shot_num=1, segment_id=0):
        ...
        if is_exemplar:
            ...
            if bounds is not None:
                ...
                # up_bound = bounds[1]//8 
                up_bound = min(math.ceil(bounds[1]/16), lim_constraint)
            # else:
            #     low_bound = 0
            #     up_bound = None
    ...
    def __getitem__(self, index):
        ...
        low = ((segment_start // 16) + (segment_id * 2)) * 16
        ...
        if self.split == 'train':
            example_rep, shot_num = self.load_tokens(examplar_path,True, cycle_start_id=cycle_start_id, count=min(actual_counts,6), shot_num=shot_num_) 
        else:
            example_rep, shot_num = self.load_tokens(examplar_path,True, id = None, shot_num=shot_num_)
        if shot_num_ == 0:
            shot_num = 0
